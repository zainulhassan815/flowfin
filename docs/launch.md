...
- ~~No launcher icon at all.~~ **Done.** The adaptive icon ships as three vector
  layers (`foreground` / `background` / `monochrome`) generated by
  `design/tools/gen_app_icon.py`, and the manifest sets `android:icon` and
  `android:roundIcon`. The "v" is the real Literata italic outline pulled from the
  font the app already bundles, so the icon and the wordmark are one letterform.
- Still missing: no 512×512 Play icon, no 1024×500 feature graphic, no screenshots.
  Those are store-listing uploads rather than APK contents, and they still gate the
  listing.
...
Every row above is now satisfied: the tile is a full-bleed rect with no baked
corners or shadow, and the mark is sized against the 72dp a launcher's mask actually
shows rather than the 108dp canvas, which keeps it inside the 66dp safe circle with
room to spare.

The warning that this was "not an export job" was right. The monogram was not
exported, it was rebuilt to Android's geometry, and the glyph is taken from
`literata_italic.ttf` at the weight and optical size brand.html specifies rather
than traced.

**brand.html is now stale on one point:** it still shows the monogram as `f·`, from
before the rename. The shipped icon is `v·`. The document should catch up.
...
| 2 | ~~No launcher icon.~~ **Done** — adaptive icon with a monochrome layer, generated from the brand monogram. Store-listing art (512×512, feature graphic, screenshots) is still outstanding. | `app/src/main/res/mipmap-anydpi-v26/`, `design/tools/gen_app_icon.py` | Listing only |
